...
UINT Adjusted_Type_Size(TY_IDX idx) {
  ...
  case KIND_STRUCT: {
	...
	if(Type_Is_Shared_Ptr(last_fld_idx) )
	  alignment = TY_align(TY_To_Sptr_Idx(last_fld_idx));
	else
	  alignment = TY_align(last_fld_idx);
      }
      if (TY_kind(last_fld_idx) == KIND_STRUCT) {
	UINT num_field = 1;
	FLD_get_to_field(last_fld_idx, 10000, num_field);
	field_id += num_field;
      }
      field_id++;
    ...
    if(field_id) {
      offset += Adjust_Field_Offset(idx, field_id-1,0) + Adjusted_Type_Size(last_fld_idx);
  ...
  if (src_real == pshared_ptr_idx) {
    ...
    if (dst_real == pshared_ptr_idx) {
      *iop = INTRN_S_TO_P;
      ...
      if (src_sz != 0 && dest_sz != 0) {
	if (src_blk != dest_blk || src_sz != dest_sz) {
	  *iop = INTRN_S_RESET;
	  result = TRUE;
	}
      }
...
INT Adjust_Field_Offset(TY_IDX struct_ty, UINT offset) 
{
  ...
  case KIND_STRUCT:
	...
	return adj_displ + items * Adjusted_Type_Size(struct_ty) + 
	  Adjust_Field_Offset(fld_ty, displ-ofst); 
      } else {
