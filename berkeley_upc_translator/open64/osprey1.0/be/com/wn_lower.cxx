...
static WN *lower_parm(WN *block, WN *tree, LOWER_ACTIONS actions){
  ...
  case OPR_MLOAD:
	...
	Is_True(0,("",""));
      ...
      if(TY_is_shared(actual_ty) && TY_kind(actual_ty) == KIND_STRUCT) {
...
static WN *lower_assert(WN *block, WN *tree, LOWER_ACTIONS actions)
{
  ...
  */
  switch ( WN_offset(tree) ) {
	 ...
	 */
#if defined(__MACH__) && defined(__APPLE__) || defined(_AIX) || defined (sun)
#define BRK_RANGE 0
#endif
...
static WN* lower_forall_ctrl(WN* block, WN* tree, WN* prag, LOWER_ACTIONS actions) {
  ...
  WN* forall_pragma = WN_prev(tree);
  FmtAssert(WN_operator(forall_pragma) == OPR_PRAGMA && WN_pragma(forall_pragma) == WN_PRAGMA_UPC_FORALL,
	    ("pragma forall should appear right before a forall loop"));
  WN* init = WN_prev(forall_pragma);
  if (init == NULL || WN_operator(init) != OPR_STID) {
    //the forall loop does contain a initialization expression
    init == NULL;
  }
