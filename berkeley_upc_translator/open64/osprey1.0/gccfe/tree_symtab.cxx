...
Get_Name (tree node)
{
	...
	if (node == NULL) {
		++anon_num;
		sprintf(buf, "anonymous%d_%ul", anon_num, hash_val(mangle_name()));
		return buf;
	}
...
Create_TY_For_Tree (tree type_tree, TY_IDX idx)
{
	...
	if (TYPE_NAME(type_tree) &&
	  ...
	  if (TY_kind(idx) == KIND_STRUCT) {
	    if (strncmp(TY_name(idx), "anonymous", 9) == 0) {
	...
	case ARRAY_TYPE:
		...
		if(TY_size(ety) == 0) {
		  if(TREE_CODE(TYPE_NAME(elem_type)) == TYPE_DECL) {
		    ...
		    error_with_file_and_line(input_filename, lineno, "Incomplete or unknown type: %s\n", Get_Name(TYPE_NAME(TREE_TYPE(type_tree))));
