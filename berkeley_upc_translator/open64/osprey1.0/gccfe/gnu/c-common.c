     ...
     int type_quals;
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE) {
    ...
    if (UPC_TYPE_USES_THREADS(type) == 1) {
      UPC_TYPE_USES_THREADS(t) = 1;
    }
...
/* 
   Wrapper to support ISO C99 of declarations in a for loop (called from c-parse.y)
*/

void handle_for_init_stmt (tree exp) {

  if (exp != NULL_TREE) {
    c_expand_expr_stmt(exp);
  }
}
