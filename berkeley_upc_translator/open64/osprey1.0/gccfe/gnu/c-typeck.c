...
extern tree build_comp_ref_shared_type(tree type);
     ...
     tree type1, type2;
{
  ...
  if(TREE_SHARED(t1) && TREE_SHARED(t2) && TREE_CODE(t1) == TREE_CODE(t2)) {
    ...
    if(bs1 != bs2 && (bs1 == NULL || bs2 == NULL))
      return 0;
    if(bs1 && bs2 && TREE_INT_CST_LOW(bs1) != TREE_INT_CST_LOW(bs2))
      return 0;
...
#define COMP_TYPE_ATTRIBUTES(t1,t2)	1
  ...
  switch (TREE_CODE (t1))
	...
	if ((! tree_int_cst_equal (TYPE_MIN_VALUE (d1), TYPE_MIN_VALUE (d2))
	    ...
	    && ((TREE_INT_CST_LOW(TYPE_MAX_VALUE(d1)) != (unsigned HOST_WIDE_INT)-1) && 
		(TREE_INT_CST_LOW(TYPE_MAX_VALUE(d2)) != (unsigned HOST_WIDE_INT)-1 ))) {
	    /* arrays with size 0 have 0xffffffff */
	  val = 0;
	}
        break;
      }
     ...
     tree type;
{
  ...
  size = get_real_size(get_TY_wrapper(type));
 
  if(is_64bit_target() && sizeof(HOST_WIDE_INT) < sizeof(long long)) {
    t = build_int_2(size&(long long)0x00000000FFFFFFFF, (size>>32)&(long long)0x00000000FFFFFFFF);
     ...
     tree datum, component;
{
  ...
  if (code == RECORD_TYPE || code == UNION_TYPE)
	...
	if (TREE_SHARED (datum)) {
	  ...
	  // and also make sure the block size is []
	  TREE_SHARED (ref) = 1;
	  if(!TREE_SHARED(field)) {
	    TREE_TYPE(ref) = build_comp_ref_shared_type(TREE_TYPE(ref));
	    /* TYPE_BLOCK_SIZE(TREE_TYPE(ref)) = build_int_2_wide(0,0); */
	  }
	}
	  ...
	  if (comp_target_types (type0, type1))
	    {
		 ...
		 warning("Undefined behavior : comparison between ptr to shared and ptr to local without a cast.");
	      result_type = common_type (type0, type1);
	...
	if (TREE_CODE (arg) == COMPONENT_REF)
	  {
	    ...
	    if (TREE_SHARED (arg)) {
	      ...
	      argtype = build_comp_ref_shared_type(argtype);
	     /*  TYPE_BLOCK_SIZE(argtype) = buuld_int_2_wide(0,0); */
  ...
  /* Conversions among pointers */
	...
	if (TREE_CODE(rhs) != INTEGER_CST || TREE_INT_CST_LOW(rhs) != 0) {
	  ...
	  if(TREE_CODE(rhs) == PLUS_EXPR) 
	    warn_for_assignment ("%s makes pointer-to-private from pointer-to-shared without a cast",
				 errtype, funname, parmnum);
	  else 
	    error ("UPC does not allow assignments"
		   " from pointer-to-private to pointer-to-shared.");
	}
     ...
     int line;
{
  ...
  if (compiling_upc) {
    if (keep_decl_for_w2c) {
      warning("asm is currently not supported in the Berkeley UPC Compiler");
    }
    return;
  }
