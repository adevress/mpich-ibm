     ...
     tree type;
{
  ...
  if (!TYPE_SHARED (type) && TREE_CODE(type) != ARRAY_TYPE)
    {
     ...
     tree exp;
{
  ...
  enum tree_code code = TREE_CODE (type);
  tree t, blocksize, totalsize, elt_size;
  ...
  elt_size = c_sizeof(get_inner_array_type(type));
  
  blks = TREE_INT_CST_LOW(blocksize);  
  ts =  get_real_size(get_TY_wrapper(type));
  esize = get_real_size(get_TY_wrapper(get_inner_array_type(type)));
  ...
  if (threads_int != 0) {
    ...
    nelem = ts/esize;
    tt = nelem/blks;
    nblks = tt / threads_int;/* num blocks per thread */
    nblks = nblks*blks; /* num elts per thread */
    r = tt * blks;
    ...
    tree S, E, E_B;
    /* dynamic env */
    /* for an array declaration shared [B] T arr[S*THREADS], with B > 0:
       let E = S / B, then  
       localsizeof(arr) = (E * B + (S - E * B) * THREADS) * sizeof (T)
     */
    if (TREE_CODE(totalsize) == MULT_EXPR) {
      totalsize = TREE_OPERAND(totalsize, 1); //get rid of the THREADS multiplier
    }

    S = size_binop(FLOOR_DIV_EXPR, totalsize, elt_size); 
    E = size_binop(FLOOR_DIV_EXPR, S, blocksize);
    E_B = size_binop(MULT_EXPR, E, blocksize);
    rem = size_binop(MINUS_EXPR, S, E_B);  /* remaining elements that goes to thread 0 */ 
    rem = size_binop(MULT_EXPR, rem, lookup_name(get_identifier("THREADS")));
    t = size_binop(PLUS_EXPR, E_B, rem);
...
tree 
build_comp_ref_shared_type(tree type) {
  ...
  case POINTER_TYPE:
    ...
    TYPE_SHARED(inner) = 1;
    TYPE_BLOCK_SIZE(inner) = build_int_2_wide(0,0);
  ...
  case ARRAY_TYPE: {
    ...
    break;
  default:
    TYPE_BLOCK_SIZE(result) = build_int_2_wide(0,0);
  } 
  return result;
}
