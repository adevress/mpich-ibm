 ...
 *   and the content should not depend on compile options
 ...
 *   upcr_trans_extra.w2c.h so they'll show up in w2c.h
 */

/* ------ Initialization callbacks that can be enabled if necessary ------ */
#if 0
  static void upcrt_proc_init() { } /* called once-per-process, before global var setup */
  void (*UPCRL_trans_extra_procinit)() = &upcrt_proc_init;
  static void upcrt_thread_init() { } /* called once-per-thread, after global var setup */
  void (*UPCRL_trans_extra_threadinit)() = &upcrt_thread_init;
#endif
