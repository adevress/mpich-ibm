 ...
 * $Id: upcr_alloc.c,v 1.91 2007/10/17 03:25:46 bonachea Exp $
 *
 */
...
void upcri_init_heaps(void *start, uintptr_t len)
{
    ...
    int i;
    static volatile int barrier_flag = 0;
                  ...
                  upcri_alloccaller_t caller)
{
	...
	if (neededlheapsz > largest_local_heapsize) {
		...
		/* some other thread may have increased our local_heapsize
	...
	upcri_assert(largest_local_heapsize >= neededlheapsz);
	lh->size = largest_local_heapsize;
	gasnet_hsl_unlock(&heapsizes_lock);
	/* It's possible that some other thread set new size while we let go
	 * of locks, and so morebytes may be 0 */
	if (morebytes) {
            if (upcri_firsttouch) /* first-touch all the pages being added */
              upcri_memtouch((void*)(lh->startaddr + (lh->size - morebytes)), morebytes, 1);
