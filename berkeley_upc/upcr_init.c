...
/* generated by linker */
extern upcri_pthreadinfo_t* upcri_linkergenerated_tld_init();
#endif
...
"-----------------------------------------------------------------------\n"
  ...
  { int threadwidth = ((int)log10(MAX(1,upcri_1stthread(gasnet_nodes()-1))))+1;
    ...
    if (upcri_mypthreads() == 1) {
      printf("UPCR: UPC thread %*i of %i on %s (process %*d of %*d, pid=%lu)\n",
	 ...
	 processwidth, (int)gasnet_mynode(), 
      ...
      printf("UPCR: UPC threads %*i..%*i of %i on %s (process %*d of %*d, pid=%lu)\n",
         threadwidth, (int)upcri_1stthread(gasnet_mynode()), 
		...
		upcr_thread_t default_pthreads_per_proc)
{
    ...
    { int politedefault = gasnett_cpu_count() > 0 && upcri_mypthreads() > gasnett_cpu_count();
      upcri_polite_wait = gasnett_getenv_yesno_withdefault("UPC_POLITE_SYNC",politedefault);
    
      if (politedefault) {
        fprintf(stderr,"WARNING: Node %i running more threads (%i) than there are physical CPU's (%i)\n",
                   (int)gasnet_mynode(), upcri_mypthreads(), gasnett_cpu_count());
...
static int startup_lvl = startup_init;
		       ...
		       const char * main_name)
{
    ...
    if (startup_lvl >= startup_done)
	return;
    if (startup_lvl != startup_init)
	upcri_early_err("upcr_startup_init called while startup_lvl=%d", startup_lvl);
    startup_lvl++;

    gasnett_maximize_rlimits();

#ifdef GASNET_SEGMENT_EVERYTHING
    upcri_err("UPCR was configured with --enable-segment-everything. This GASNet segment mode is currently not supported by Berkeley UPC.");
#endif
 ...
 */
static void upcri_atexit_handler()
{
			 ...
			 int flags)
{
    ...
    if (startup_lvl >= startup_done)
	return;
    if (startup_lvl != startup_attach)
	upcri_err("upcr_startup_attach called while startup_lvl=%d", 
		  startup_lvl);
    startup_lvl++;
...
"upcr_startup_attach: default_shared_offset %lu not a multiple of UPCR_PAGESIZE",
    ...
    upcri_thread2local = UPCRI_XMALLOC(uintptr_t, upcri_threads);
#endif
...
"Howver, node %d was only able to allocate %lu MB of shared memory.\n"
"\n"
...
"UPC_SHARED_HEAP_SIZE or UPC_SHARED_HEAP_OFFSET environment variables.\n",
	...
	if (smallest_perthread < perthread_req) {
	    if (gasnett_getenv_yesno_withdefault("UPC_NO_SIZE_WARN",flags & UPCR_ATTACH_SIZE_WARN)) {
     ...
     * address if local.
     *
     * TODO:  SysV will need more logic 
     */
    bzero(upcri_thread2local, sizeof(uintptr_t) * upcri_threads);

    #if UPCRI_UPC_PTHREADS
	t = upcri_1stthread(gasnet_mynode());
	mymaxthread = t + upcri_mypthreads() - 1;
	while (t <= mymaxthread) {
	    #if UPCR_USING_LINKADDRS
		upcri_thread2local[t] = upcri_linkoffset[t];
	    #else
		upcri_thread2local[t] = upcri_thread2region[t];
	    #endif
	    t++;
	}
    #elif UPCRI_SYMMETRIC_SEGMENTS
       /* configs with load/store access to remote shared memory - 
          shmem, global pointers, and possibly sysv ? */
	for (n = 0; n < nodes; n++) {
          #if UPCR_USING_LINKADDRS
            upcri_thread2local[n] = upcri_linkoffset[n];
          #else
	    upcri_thread2local[n] = (uintptr_t) seginfos[n].addr;
          #endif
	}
    #else /* !UPCRI_SYMMETRIC_SEGMENTS && !UPCRI_UPC_PTHREADS */
        /* unaligned segment platforms with exactly one thread per node */
      #if UPCR_USING_LINKADDRS
        upcri_thread2local[gasnet_mynode()] = upcri_linkoffset[gasnet_mynode()];
      #else
        upcri_thread2local[gasnet_mynode()] = (uintptr_t) seginfos[gasnet_mynode()].addr;
      #endif
    #endif
    #if UPCRI_SYMMETRIC_SEGMENTS 
    ...
    */
    ...
    { const char *trace_id;
      ...
      if (!trace_id) { /* no id available from upcrun, so have thread 0 generate one based on hostname and pid */
        ...
        sprintf(tmp, "%llu", (unsigned long long)id);
        trace_id = tmp;
      }
    ...
    /* XXX: Bind pthreads to CPUs if possible */
			...
			struct upcr_startup_spawnfuncs *spawnfuncs)
{
    ...
    if (startup_lvl >= startup_done)
	return;
    if (startup_lvl != startup_spawn)
	upcri_err("upcr_startup_spawn called while startup_lvl=%d", startup_lvl);
    startup_lvl++;
...
static void upcri_signal_finalbarrier() {
  UPCR_BEGIN_FUNCTION();
  /* send a final barrier confirmation to zero containing the phase we'll have after final barrier */
...
static void upcri_check_finalbarrier() {
  UPCR_BEGIN_FUNCTION();
  int i;
  int threads = upcr_threads();
  int myphase = (upcri_auxdata()->barrier_info.barrier_phase);
  ...
  if (p != desc)
    upcri_err("Early exit detected: Thread %i reached the final implicit barrier, "
              "but the following threads did not: %s", 
              (int)upcr_mythread(), desc);
  upcri_free(tmp);
  upcri_free(desc);
}
...
upcri_do_exit(int exitval)
{
     ...
     * that's the case
     * XXX: Must NOT use UPCRI_SINGLE_BARRIER*() to ensure these remain anon.
     */
    upcr_notify(0, UPCR_BARRIERFLAG_ANONYMOUS);
    upcr_wait(0, UPCR_BARRIERFLAG_ANONYMOUS);
    {
      /* sleep for 1 second, to flush out stdio/stderr
       * - TODO: this may not really be needed anymore (see bug 1785) */
      gasnett_tick_t starttime = gasnett_ticks_now();
      GASNET_BLOCKUNTIL(gasnett_ticks_to_us(gasnett_ticks_now()-starttime)>1000000);
    }
     ...
     * partial thread early-exit bugs */
    if (upcr_mythread() == 0) {
      upcri_check_finalbarrier();
      upcr_global_exit(exitval);
    } else {
      /* wait for thread 0 to kill us */
      /* choose a reasonably large timeout value to prevent false negatives at large scale */
      int waittime = (5+gasnet_nodes()/64)*1E6; 
      gasnett_tick_t start = gasnett_ticks_now();
      while (gasnett_ticks_to_us(gasnett_ticks_now() - start) < waittime) {
        upcr_poll();
        gasnett_sched_yield();
      }
      /* hmm.. we're still alive - probably means thread zero is not here,
       * ie this is an erroneous program and we're an early exit thread */
      upcri_check_finalbarrier();
      /* if we returned from above then the exit is collective, but zero 
       * somehow failed to kill the job, so kill ourselves
       */
      upcr_global_exit(exitval);
    }
}
...
void 
upcr_startup_initarray(upcr_shared_ptr_t dst, void * src, 
		       upcr_startup_arrayinit_diminfo_t *diminfos, 
		       size_t dimcnt, size_t elembytes, size_t blockelems)
{
    upcri_err("static initialization of shared arrays is not yet supported");
}

void 
upcr_startup_initparray(upcr_pshared_ptr_t dst, void * src, 
			upcr_startup_arrayinit_diminfo_t *diminfos, 
			size_t dimcnt, size_t elembytes, size_t blockelems)
{
    upcri_err("static initialization of shared arrays is not yet supported");
