...
# directories
my ($cwd, $in_build_tree, $upcr_home, $upcr_bin, $upcr_etc, $upcr_lib, $upcr_include, $upcr_share);
...
EOF
    unless ($h2mhelp) {
        print <<EOF;
For detailed documentation, please see man upcrun(1) or http://upc.lbl.gov/docs/
EOF
    }
    print <<EOF; 
...
Options:
    ...
    -[no]trace
            ...
            executable was build against a runtime with tracing
            enabled.
    -traceall
...
sub gasnet_encode {
    ...
    $in =~ s/([^A-Za-z0-9%_,\.\/:+=@^-])/sprintf("%%0%02x",(ord($1)))/ge;
    return $in;
}
...
sub initialize 
{
    ...
    if (-f "$upcr_bin/upcrun.conf") {
        $upcr_home = $upcr_etc = $upcr_lib = $upcr_include = $upcr_share = $upcr_bin;
	$in_build_tree = 1;
    } else {
    ...
    # read ad validate config file(s)
    readconfig();
    ...
    if ($gasnet_debug && !$ENV{MALLOC_CHECK_}) {
	# for debug builds, enable the Linux debug malloc library by default
	pushVar('MALLOC_CHECK_',2) unless ($$upcrexe_miscinfo{UPCRBinaryInterface} =~ /-blrts-/);
    }
	...
	$default_shared_heapsz_max =~ s/\(u?int(32|64)_t\)//g;
	...
	# for smp conduit, the only parallelism is pthreads, so set the 
 	# UPC thread count to the pthread count if there is no other source.
	if ($opt_numthreads) {
	  # Ensure pthreads is present if > 1 thread requested
	  if (($opt_numthreads > 1) && !isPthreads()) {
	    die "When network=smp compile with '-pthreads' to run with > 1 thread\n";
	  }
	} else {
    ...
    if (defined $shared_heapsz_max &&
        parse_size($shared_heapsz_max) < $opt_th_per_proc*parse_size($shared_heapsz)) {
        $opt_shared_heap_max = size_str($opt_th_per_proc*parse_size($shared_heapsz),1);
    ...
    if ($opt_localhost) {
        ...
        # UDP on a single host is essentially a toy (smp should always be
        # faster).  So assume user may also be oversubscribing CPUs, and avoid
        # embarrasingly slow spin-lock performance.
        pushVar("UPC_POLITE_SYNC", "1");
    } else {
        ...
        # $TMPDIR/machines or $TMP/machines when PE_HOSTFILE is set
        ...
        if (!$UPC_NODES && $ENV{PE_HOSTFILE}) {
	    # Look for files provided by Grid Engines's mpich support
	    my @tmp_list = qw(mpi vapi ibv);
            if ($ENV{TMPDIR} && -f "$ENV{TMPDIR}/machines") {
                $UPC_NODES = parseNodeFile("$ENV{TMPDIR}/machines");
                $recognized_file = grep($_ eq $conduit, @tmp_list);
            } elsif ($ENV{TMP} && -f "$ENV{TMP}/machines") {
                $UPC_NODES = parseNodeFile("$ENV{TMP}/machines");
                $recognized_file = grep($_ eq $conduit, @tmp_list);
            }
        }
        ...
        if ($UPC_NODES) {
          ...
          if (($conduit eq 'gm' || $conduit eq 'mpi')
            ...
            foreach my $node (split(' ',$UPC_NODES)) {
                print $fh "$node\n";
            }
            undef $fh;
...
sub parseExecutable {
  ...
  if ($gasnetlib_ctuple =~ /CONDUIT=([A-Za-z]+)/) {
    $conduit = lc($1);
  } else {
    die "'$exename' is missing network conduit information\n";
  }
  unless (grep { /$conduit/ } @conduits ) {
...
sub readconfig {
    ...
    %conf = parseconfig($upcrun_conf, 1, %conf);
    
    # Users may also put prefs in a ~/.upcrunrc file 
    my $upcrunrc = ($ENV{HOME} || (getpwuid($<))[7]) . '/.upcrunrc';
    %conf = parseconfig($upcrunrc, 0, %conf) if -f $upcrunrc;
...
sub runDriver
{
	...
	if ($numnodes > 1) {
	    $full_nodes = $opt_numthreads - $numnodes * int($opt_numthreads / $numnodes);
	...
	# Layout nodes that have less than full thread count
        {
	    my $threads = $opt_numthreads - $full_nodes * $cpus_per_node;
	    my $quotient = int($threads / $opt_th_per_proc);
	    my $remainder = $threads - ($quotient * $opt_th_per_proc);
	    my $is_remainder = $remainder ? 1 : 0;
    ...
    if ($opt_encode_env) {
	  ...
	  pushVar($varname, $encval) if ($encval ne $val);
	}
      }
      pushVar('GASNET_DISABLE_ENVDECODE', 0) if (defined $ENV{'GASNET_DISABLE_ENVDECODE'});  
    }
    if ($opt_encode_args) {
      map { $_ = gasnet_encode($_); } @exeargs;
      pushVar('GASNET_DISABLE_ARGDECODE', 0) if (defined $ENV{'GASNET_DISABLE_ARGDECODE'});  
    }
    ...
    my @spawncmd = map {  if (m/^%[Aa]$/) {
			    ...
			    while (m,###INSTALL_PREFIX###/bin/(\S+)\b,) {
			    ...
			    s/%[Dd]/$tmp_cwd/g; 
			    s/%[Nn]/$numproc/g; 
			    s/%[Mm]/$numnodes/g; # XXX prohibit when not using <conduit>_spawn_nodes ??
