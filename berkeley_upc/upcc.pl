...
# if skipping $HOME/.upccrc
my ($opt_norc);

# directories
my ($startdir, $trans, $upcr_home, $upcr_bin, $upcr_etc, $upcr_lib,
    $upcr_include, $upcr_include_src);
...
# set to 'seq', 'par', or 'tv', depending on use of pthreads
my $parseq;
...
$extra_help
UPC-related options:
   -network=<type>    Set network API use for communication.  Valid types include:
                       $network_str
...
Advanced options:
   ...
   -print-include-dir Prints full path to directory in which <bupc_extern.h>
                      is located.
   -print-mpicc       Prints full pathname of a MPI compiler compatible with this
                      installation of upcc, or error if MPI not supported.
   -inst[-local,-functions]   Used internally by GASP performance tool wrapper scripts
   -pthreads[=N]      Generate a pthreaded UPC executable, and optionally embed
   ...
   -[no]save-all-temps  Save all files used during translation/compilation.
   ...
   -Wp,<option>       Pass arbitrary option to the back-end C preprocessor: 
                      use repeatedly to pass multiple options.  If you need to
                      use spaces, quote the option (ex: -Wp,"--option value").
   -Wu,<option>       Pass arbitrary option to sgiupc: use repeatedly to 
                      pass multiple options.  If you need to use spaces, quote
                      the option (ex: -Wu,"--option value").
   -Ww,<option>       Pass arbitrary option to whirl2c translator: use
                      repeatedly to pass multiple options.  If you need to use
                      spaces, quote the option (ex: -Ww,"--option value").
   -Wc,<option>       Pass arbitrary option to the back-end C compiler: use
                      repeatedly to pass multiple options.  If you need to use
                      spaces, quote the option (ex: -Wc,"--option value").
   -Wl,<option>       Pass arbitrary option to the back-end C linker: use
                      repeatedly to pass multiple options.  If you need to use
                      spaces, quote the option (ex: -Wl,"--option value").
...
sub get_make_value
{
    ...
    my $cmd = qq|$gmake -f $upcr_include/upcc.mak echovar VARNAME=$varname|
...
sub get_gccupc_compiler_version
{
    ...
    my $trans_spec = "(N/A)";
    my $gccupc_v = `/bin/sh -c "$trans -v 2>&1"`;
    if ($gccupc_v =~ /(gcc version.*\(GCC UPC.*\))/) {
        $trans_version = $1;
    }
    if (my @stat = stat($trans)) {
...
sub initialize 
{
    ...
    # search for config files depends on the -at-remote-ssh and -norc options.
    # This little trick lets us parse these two options from the
    ...
    $upcr_include_src = $conf{'top_srcdir'} if (!defined($upcr_include_src));
    ...
    if ($ENV{UPCC_FLAGS}) { 
      ...
      if ((my $invalid) = grep /^-?-norc/, @env_args) {
    ...
    if ($conf{"${opt_network}_options"}) { 
      ...
      if ((my $invalid) = grep /^-?-(network|norc)/, @conduit_args) {
    ...
    if ($conf{default_options}) { 
      ...
      if ((my $invalid) = grep /^-?-norc/, @default_args) {
...
#        'W=s'              # handled by do_ARGVoodoo
    ...
    # GCCUPC doesn't work with lots of our options
    $gccupc = 1 if $conf{using_gccupc} =~ /^y/i;
    if ($gccupc) {
        my $errs = '';
        for ( [$opt_yesterday, '--yesterday'],
              [$opt_nightly, '--nightly'],
              [$opt_stable, '--stable'],
              [$opt_transopt, '--opt'],
              [$opt_translateonly, '--trans'],
            ) 
        {
            if ($$_[0]) {
                $errs .= "'$$_[1]' does not work with GCC UPC-based upcc\n      ";
            }
        }
        if ($errs) {
            $errs =~ s/ +$//;
    ...
    if ($gccupc) {
	if ($trans =~ m!^http://! || $trans =~ m!^[^/]*:.*!) {
	    die "'translator=$trans'\n  remote translation not supported for GCC UPC-based upcc";
	}
        if ($opt_pthreads && $conf{gccupc_tls} !~ /yes/i) {
            die "this configuration does not support pthreads (no GCC '__thread' attribute)\n";
        }
        unless (-f $trans) {
       	    die "UPC translator '$trans' does not exist!\n";
        }
    }
    ...
    unless (-d $trans || $use_nettrans_http || $gccupc
            || ($use_nettrans_ssh && !$opt_ssh_remote)) {
        die "UPC compiler directory '$trans' is not a directory!\n"
    }
    ...
    if ($opt_inst || $opt_inst_local || $opt_inst_functions) { 
      ...
      my $inst_flags = ($gccupc ? \@opt_CPP_args : \@opt_CC_args);
	...
	close (SIZES);
        ...
        for (qw/shared_ptr pshared_ptr mem_handle reg_handle void_ptr ptrdiff_t int 
                char short long longlong float double longdouble size_t/) 
        {
            my $size = $$sizes_ref{$_};
    ...
    $extra_cppflags = "@opt_CPP_args -D__BERKELEY_UPC_RUNTIME__=1";
    ...
    # UPC language defines (which GCC/UPC provides for itself)
    ...
    unless ($gccupc) {
        # standardized features available in Berkeley UPC
	...
	# berkeley-specific defines
        ...
        if ($opt_pthreads) {
            $extra_cppflags .= " -D__BERKELEY_UPC_PTHREADS__=1";
        }
	if ($opt_pow2_symptr) {
	    $extra_cppflags .= " -D__BERKELEY_UPC_POW2_SYMPTR__=1";
	}
...
sub do_ARGVoodoo {
	...
	my $variety = $1;
        ...
        ### handle gcc-style '-Wu,--flag=foo,-bar' type args
        if ($arg =~ /^-Wp,(.*)$/) {
            push @opt_CPP_args, $1;
        } elsif ($arg =~ /^-Wu,(.*)$/) {
            push @opt_UPC_args, $1;
...
sub readconfig {
    ...
    %conf = parseconfig($upcc_conf, 1, %conf);
    # Users may also put prefs in a ~/.upccrc file (which we'll have copied to
    # the current directory if we're doing a nettrans_ssh)
    unless ($opt_norc) {
        my $upccrc = $opt_ssh_remote ? ".upccrc" : "$ENV{HOME}/.upccrc";
        %conf = parseconfig($upccrc, 0, %conf) if -f $upccrc;
    }
...
#   my $exitval = system(qq{/bin/sh -c "$cmd >$out 2>$err"});
    ...
    if ("$conf{warning_blacklist}" ne "" && !$opt_verbose) {
      $errtxt =~ s/$conf{warning_blacklist}//mg;
    }
...
sub runDriver
{
    ...
    for my $file (@ARGVoodoo) {
        ...
        if ($file =~ /[$unsupported_filename_chars\[\]\\*]/o) {
            die "'$file': upcc does not support files containing any of "
...
sub do_preprocess
{
    ...
    foreach my $basename (@toPreprocess) {
	    ...
	    # DOB: UPC spec requires these available without upc.h
	...
	close(DST);
        ...
        my $if_gccupc;
        if ($gccupc) {
            $if_gccupc = "USE_GCCUPC_COMPILER=yes GCCUPC=\"$trans\"";
            $detect_upc_flags .= " -b"; # use 'both' C and UPC pragmas for gccupc
            # Manually include flags needed to include <upcr.h> and
            # dependencies
            $xtraflags .= ' \$(UPCR_INCLUDES) \$(GASNET_CPPFLAGS)';
        ...
        $cmd = qq|$gmake -f $upcr_include/upcc.mak|
             . qq| EXTRA_CPPFLAGS="$extra_cppflags -I\'$abshome\' $xtraflags"|
             . qq| UPCPPP_INCLUDEDIRS="$Ipaths"|
             . qq| DETECT_UPC_FLAGS="$detect_upc_flags"|
             . qq| UPCR_CONDUIT=$opt_network UPCR_PARSEQ=$parseq $if_gccupc|
        ...
        if ($opt_save_temps) {
            runCmd("cp \'$tmp/$ifile\' \'$startdir\'", 
                "error saving preprocessor output for -save-temps");
        }
...
sub do_translate
{
	...
	for ( ; $retry <= $retries; $retry++) {
	    ...
	    my $iaddrstr = join(".",unpack('C4',$iaddr)).":".$iport;
	...
	if ($contenttype =~ "^text/plain") {
        ...
        close REMOTE_LOG; 
        if ("$conf{warning_blacklist}" ne "" && !$opt_verbose) {
          $err =~ s/$conf{warning_blacklist}//mg;
        }
        print STDERR $err;
        ...
        my $upccrc_quoted = "";
        unless ($opt_norc) {
            $upccrc_quoted = "'$ENV{HOME}/.upccrc'" if -r "$ENV{HOME}/.upccrc";
        }
...
EOF
	    ...
	    my $killsrcpos = 0;
		  ...
		  my $line = '';
		  while (/\bbupc_mangled_va_(start|arg)\s*\(\s*/) {
		    my $kind = $1;
		    $line .= $`;
		    $_ = $';
...
sub do_compile
{
    ...
    foreach my $basename (@toCompile) {
        ...
        my $in = $gccupc ? "$tmp/$basename.i" : "$tmp/$basename.trans.c";
        check_ctuple_trans("$in") if $opt_checks;
        validate_spec_version("$in") unless $gccupc;

        if (!$gccupc) { # find the translator-generated ALLOC/INIT functions
          my %fns;
        ...
        if ($opt_pthreads && !$gccupc) {
            ...
            $cmd = "cp \'$in\' \'$out\'";
            runCmd($cmd, "error while copyng object file");
        } else {
...
sub do_link
{
    ...
    # if pthread, we need to actually compile the objects (unless using gccupc)
    if ($opt_pthreads && !$gccupc) {
        for (@toLinkPthreads) {
    ...
    if ($gccupc) {
	# UPC cpp flags omitted when invoking GCC/UPC, but need them for bootstrap C file
...
sub create_bootstrap_file {
    ...
    foreach my $obj (@toLink) {
      ...
      foreach my $alloc (@allocs) {
        $alloc_protos .= "void $alloc();\n";
        $alloc_calls .= "    $alloc();\n";
      }
      foreach my $init (@inits) {
        $init_protos .= "void $init();\n";
        $init_calls .= "    $init();\n";
      }
    }
    # Berkeley UPC translator transforms 'main' into 'user_main': 
    # The gccupc compiler uses 'upc_main'
    my $main_name = ($gccupc ? "upc_main" : "user_main");
...
EOF
    ...
    if ($gccupc) {
        print LINK_C <<EOF;
void gccupc_pre_spawn_init();
void gccupc_per_pthread_init();
...
void (*UPCRL_pre_spawn_init)()                       = $pre_spawn_init_func;
void (*UPCRL_per_pthread_init)()                     = $per_pthread_init_func;
...
static void maybe_MPI_Finalize()
{
    int err;
...
void (*UPCRL_mpi_finalize)() = &maybe_MPI_Finalize;
...
EOF
  ...
  void (*UPCRL_profile_finalize)() = &upcri_do_mcleanup;
#endif
EOF
   }
...
sub is_upc_obj {
    my ($obj) = @_;
    if (1) { # ctuples work for both text and binary
      my ($gasnet_ctuples,$upcr_ctuples) = extract_ctuples($obj);
      return scalar(keys %{$upcr_ctuples});
    } else { # old code requires nm, which may be missing
      my $nm = verify_exec($conf{nm}, $conf{exe_suffix});
      # Some versions of grep don't work with binary files, so try nm first.
      my $err = system(
        "/bin/sh -c \"$nm \'$obj\' 2>/dev/null | grep UPCRI_IdentString_ >/dev/null 2>/dev/null\"");
      if ($err) {
        # try grep: for pthreads
        $err = system("grep UPCRI_IdentString_ \'$obj\' >/dev/null 2>/dev/null");
      }
      return ($err == 0);
    }
}
...
sub is_upc_pthreads_obj {
  ...
  } else { # BUPC pthreads objects are just source
    my $err = system(
      "grep 'UPC Runtime specification expected:' \'$obj\' >/dev/null 2>/dev/null");
