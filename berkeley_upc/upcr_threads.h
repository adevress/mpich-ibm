...
/* stack-checking support */
  ...
  #define UPCRI_STACK_CHECK_DECLNOSEMI() \
    ; const char _bupc_dummy_STACKCHECK = (upcri_stack_check(&(upcri_auxdata()->stack_info), \
                                           upcr_mythread(), __FILE__, __LINE__),0) /* intentional no semicolon */
#else
  #define UPCRI_STACK_CHECK_DECLNOSEMI() 
#endif

#ifndef UPCRI_STACK_DEFAULT
#define UPCRI_STACK_DEFAULT (1*1048576) 
#endif
...
typedef struct upcri_lockinfo {
    volatile int upclock_state;	/* state of upc lock algorithm for this thread */
    volatile upcr_thread_t upclock_handoffarg;
#if UPCR_DEBUG || UPCR_LOCK_DEBUG
    void *upclock_locksheld;	/* for debugging purposes only */
    gasnet_hsl_t locksheld_hsl; /* protects list above */
#endif
#ifdef GASNET_STATS
    int64_t alloccnt;
    int64_t freecnt;
...
typedef struct {
  ...
  char _pad2[GASNETT_CACHE_LINE_BYTES];
...
#if UPCRI_UPC_PTHREADS && !UPCRI_USING_GCCUPC
...
/******************************************************************************/
  ...
  #define UPCRI_PT_ARG_ALONE  upcri_pthreadinfo_t * const UPCR_RESTRICT _upcr_pthreadinfo
  ...
  #define UPCRI_PTHREADINFO_VALID()      (_upcr_pthreadinfo != NULL)
    ...
    #define UPCRI_ASSERT_NONNULL_DECLNOSEMI(expr) \
      ; const char _bupc_dummy_NONNULL = (upcri_assert(expr != NULL),0) /* intentional no semicolon */
  #else
    #define UPCRI_ASSERT_NONNULL_DECLNOSEMI(expr) 
  #endif
  ...
  #define UPCRI_PT_ARG_ALONE  
  #define UPCRI_PT_ARG
  #define UPCRI_PT_PASS_ALONE
  #define UPCRI_PT_PASS

  #define UPCRI_PTHREADINFO_LOOKUPDECL() const char _bupc_dummy_PTHLOOKUP = 0
  #ifdef UPCRI_UPC_PTHREADS
  ...
  #define UPCR_BEGIN_FUNCTION()                                        \
    UPCRI_ASSERT_NOTINAMHANDLER_DECL()  /* intentional no semicolon */ \
    int _bupc_dummy_BEGIN_FUNCTION = 0  /* intentional no semicolon */ \
    upcri_srcpos_declnosemi()           /* intentional no semicolon */ \
    UPCRI_PEVT_BEGINFN_DECLNOSEMI()     /* intentional no semicolon */ \
    UPCRI_STACK_CHECK_DECLNOSEMI()      /* intentional no semicolon */ \
    UPCRI_ALLOCA_DECLNOSEMI()           /* intentional no semicolon */
