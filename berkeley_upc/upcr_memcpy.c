/*   $Source: /var/local/cvs/upcr/upcr_memcpy.c,v $
 *     $Date: 2007/02/27 14:04:00 $
 * $Revision: 1.12 $
 * Description: 
...
extern void _bupc_waitsync_all (bupc_handle_t *_ph, size_t _n UPCRI_PT_ARG) {
  ...
  for (i=0; i < _n; i++) {
    _bupc_waitsync(_ph[i] UPCRI_PT_PASS);
...
extern int  _bupc_trysync_all  (bupc_handle_t *_ph, size_t _n UPCRI_PT_ARG) {
  ...
  for (i=0; i < _n; i++) {
    ...
    else if (_bupc_trysync(h UPCRI_PT_PASS)) { /* found complete */
      _ph[i] = BUPC_COMPLETE_HANDLE;
...
extern int  _bupc_trysync_some (bupc_handle_t *_ph, size_t _n UPCRI_PT_ARG) {
  ...
  for (i=0; i < _n; i++) {
    ...
    if (h != BUPC_COMPLETE_HANDLE) {
      empty = 0;
      if (_bupc_trysync(h UPCRI_PT_PASS)) { /* found complete */
        _ph[i] = BUPC_COMPLETE_HANDLE;
        success = 1;
      }
    }
...
extern bupc_handle_t _bupc_memcpy_async(upcr_shared_ptr_t dst, upcr_shared_ptr_t src, size_t n UPCRI_PT_ARG) {
  UPCRI_EOP_ENCAPSULATE(_upcr_nb_memcpy(dst, src, n UPCRI_PT_PASS));
}

extern bupc_handle_t _bupc_memget_async(void *dst, upcr_shared_ptr_t src, size_t n UPCRI_PT_ARG) {
  UPCRI_EOP_ENCAPSULATE(_upcr_nb_memget(dst, src, n UPCRI_PT_PASS));
}

extern bupc_handle_t _bupc_memput_async(upcr_shared_ptr_t dst, const void *src, size_t n UPCRI_PT_ARG) {
  UPCRI_EOP_ENCAPSULATE(_upcr_nb_memput(dst, (void *)src, n UPCRI_PT_PASS));
}

extern bupc_handle_t _bupc_memset_async(upcr_shared_ptr_t dst, int c, size_t n UPCRI_PT_ARG) {
...
/*---------------------------------------------------------------------------------*/
  ...
  extern bupc_handle_t _bupc_memcpy_fstrided_async(upcr_shared_ptr_t dstaddr,  size_t dstchunklen, 
    ...
    UPCRI_PASS_GAS();
    upcri_checkvalid_shared(srcaddr);
    upcri_checkvalid_shared(dstaddr);
