...
void printtree(nodeptr, unsigned);
void dumpCells(int ProcessId) {
  
  cellptr *cc,q;
  nodeptr r;
  int i;
  for (cc = Local.mycelltab+Local.myncell-1; 
	cc >= Local.mycelltab; cc--) {
     q = *cc;
     for (i = 0; i < NSUB; i++) {
       r = Subp(q)[i];
       if(ProcessId == 0)
	 printf("%d: HACKCOFM ITERATION START %d 0x%llx\n", ProcessId, i, r); 
     }
   }
}
...
bool intcoord(xp, rp)
  ...
  shared [] double *rp;                     /* real coordinate vector (system coords) */
{
   int k;
   bool inb;
   double xsc, floor();
   ...
   unsigned ProcessId;
{
   ...
   valid_root = TRUE;
   for (i = 0; i < NDIM; i++) {
      xor[i] = xp[i] ^ Local.Root_Coords[i];
   }
   ...
   qptr = (shared nodeptr *) (Subp(mynode))+ kidIndex;
   l = Level(mynode) >> 1;
   flag = TRUE;
   ...
   while (flag) {                           /* loop descending tree     */
	 ...
	 upc_lock(CellLock[((cellptr) mynode)->seqnum % MAXLOCK]);
	 if (*qptr == NULL) {
	   ...
	   le->num_bodies = le->num_bodies+1;
	   *qptr = (nodeptr) le;

	   flag = FALSE;
	 }
	 upc_unlock(CellLock[((cellptr) mynode)->seqnum % MAXLOCK]);
	 ...
	 qptr = (shared nodeptr *) Subp(*qptr) + kidIndex;  /* move down one level  */
	 l = l >> 1;                            /* and test next bit    */
	 
      }
   }
   SETV(Local.Root_Coords, xp);
   mynode =  Parent((leafptr) *qptr);
   return mynode;
}
   ...
   for (cc = Local.mycelltab+Local.myncell-1; 
	cc >= Local.mycelltab; cc--) {
	 ...
	 if (r != NULL) {
	    ...
	    Mass(q) = Mass(q) +  Mass(r);
	    Cost(q) = Cost(q) + Cost(r);   
   ...
   unsigned ProcessId;
{
   ...
   Local.Current_Root = (nodeptr) G_root;
   
   for (pp = Local.mybodytab; 
	pp < Local.mybodytab+Local.mynbody; pp++, count++) {
      p = *pp;
      if (Mass(p) != 0.0) {
	 Local.Current_Root 
	    = (nodeptr) loadtree(p, (cellptr) Local.Current_Root, 
				 ProcessId);
      }
      else {
	 upc_lock(io_lock);
	 fprintf(stderr, 
		 "%d: Process  found body %d at 0x%llx to have zero mass \n",
		 MYTHREAD, count,  p);	
	 upc_unlock(io_lock);
      }
   }
      ...
      unsigned ProcessId;
{
   ...
   switch (Type(n)) {
    case CELL:
      c = (cellptr) n;
      nseq = c->seqnum;
      printf("%d: Cell : Cost = %d, ", ProcessId, Cost(c));
      PRTV("Pos", Pos(n));
      printf("\n");
      for (k = 0; k < NSUB; k++) {
	 printf("%d: Child #%d: ", ProcessId, k);
	 if (Subp(c)[k] == NULL) {
	    printf("NONE");
	 }
	 else {
    ...
    case LEAF:
      l = (leafptr) n;
      nseq = l->seqnum;
      printf("%d: Leaf : # Bodies = %2d, Cost = %d, ", ProcessId, l->num_bodies, Cost(l));
      PRTV("Pos", Pos(n));
      printf("\n");
      for (k = 0; k < l->num_bodies; k++) {
	 p = Bodyp(l)[k];
	 printf("%d: Body #%2d: Num = %2d, Level = %o, ", ProcessId,
		p - bodytab, k, Level(p));
	 PRTV("Pos",Pos(p));
	 printf("\n");
      }
