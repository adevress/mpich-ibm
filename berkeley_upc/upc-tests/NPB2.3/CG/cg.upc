...
/* Initialize block Jacobi preconditioner (with constant blocks)
 */
poisson_jacobi_t *poisson_jacobi_init(int block_size)
{
    double *diag;
    int i;

    poisson_jacobi_t *M =
        (poisson_jacobi_t *) malloc(sizeof(poisson_jacobi_t));

    M->block_size = block_size;
    M->data = (double *) malloc(2 * block_size * sizeof(double));
    ...
    band_cholesky_factor(block_size, 1, M->data, 2);
    return M;
}


/* Block Jacobi preconditioner (M = diag blocks of A)
 */
void poisson_jacobi_psolve(double *Minvx, void *Mdata, double *x, int n)
{
    poisson_jacobi_t *M = (poisson_jacobi_t *) Mdata;
    int block_size = M->block_size;
    int i;

    /* Apply the block preconditioner */
    memcpy(Minvx, x, n * sizeof(double));
    for (i = 0; i < n; i += block_size) {
        int this_block_size = min(block_size, n - i);
        double *xblock = &(Minvx[i]);
        band_cholesky_solve(this_block_size, 1, 1,
                            M->data, 2, xblock, block_size);
    }
}
...
csr_matrix_t *csr_mm_upc_load(char *filename)
{
    ...
    static shared int n;
    static shared int row_start[10000];
    static shared int col_idx[100000];
...
{
    csr_jacobi_t *M = (csr_jacobi_t *) malloc(sizeof(csr_jacobi_t));

    int n = A->m;
    int *Arow = A->row_start;
    int *Acol = A->col_idx;
    double *Aval = A->val;
    int my_start = A->start[MYTHREAD];

    int nblocks = (n + block_size - 1) / block_size;
    int blockbytes = block_size * block_size * sizeof(double);
    double *current_block;
    int k;

    M->n = n;
    M->block_size = block_size;
    M->factored_blocks = (double *) malloc(nblocks * blockbytes);
    if (M->factored_blocks == NULL) {
      printf("THREAD %d: M->factored_block == NULL\n", MYTHREAD);
      upc_global_exit(1);
    }

    current_block = M->factored_blocks;
    for (k = 0; k < n; k += block_size) {
        int block_end = min(n, k + block_size);
        int current_block_size = block_end - k;
        int i;

        memset(current_block, 0, blockbytes);

        /* Pack a diag block into col-major dense format */
        for (i = k; i < block_end; ++i) {
            int blocki = i - k;
            int j = Arow[i];
            for (; j < Arow[i + 1]; ++j) {
                int blockj = Acol[j] - k - my_start;
                if (blockj >= 0 && blockj < block_size) {
                    current_block[blockj * block_size + blocki] = Aval[j];
                }
            }
        }

        /* Do a Cholesky factorization of the diag block */
        cholesky_factor(current_block_size, current_block, block_size);
...
/* Solve M\x where M is a block-Jacobi preconditioner matrix
 */
void csr_jacobi_psolve(double *Minvx, void *Mdata, double *x, int n)
{
    csr_jacobi_t *M = (csr_jacobi_t *) Mdata;
    int block_size = M->block_size;
    double *current_block = M->factored_blocks;
    int k;
    ...
    for (k = 0; k < n; k += block_size) {
        ...
        int block_end = min(n, k + block_size);
        int current_block_size = block_end - k;
        double *xblock = &(Minvx[k]);

        cholesky_solve(current_block_size, 1,
                       current_block, block_size,
                       xblock, current_block_size);
...
int main(int argc, char **argv)
{
    ...
    if (argc <= 1) {
        ...
        poisson_jacobi_t *M = poisson_jacobi_init(block_size);
	...
	timer_start(0);
        driver(start, 500, poisson_matvec, NULL, poisson_jacobi_psolve, M);
	timer_stop(0);
	timer_start(1);
        driver(start, 500, poisson_matvec, NULL, dummy_psolve, NULL);
	timer_stop(1);
	printf("time for 500 possion matrix without precond: %f sec\n", timer_read(1));
	printf("time for 500 possion matrix with precond: %f sec\n", timer_read(0));
    } else {
        int block_size = 60;
	csr_matrix_t *A;
        csr_jacobi_t *Mj;
        ...
        if (MYTHREAD == 0) 
            printf("Initializing preconditioner...\n");
        Mj = csr_jacobi_init(A, block_size);
	...
	timer_start(0);
        driver(A->start, A->n, csr_matvec, A, csr_jacobi_psolve, Mj);
	timer_stop(0);
	printf("time for %s with block jacobi: %f sec\n", argv[1], timer_read(0));
