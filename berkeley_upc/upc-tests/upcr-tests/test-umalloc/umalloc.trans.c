...
void
twoheap_sizes_test(umalloc_heap_t *upheap, umalloc_heap_t *downheap, int iters)
{
    void      *allocs[SIZES_NUM*2];
    int	       allocsz[SIZES_NUM*2];
    ...
    srand(0);	/* We still want a repeatable sequence, for debugging. */
...
{ UPCR_BEGIN_FUNCTION();
    ...
    area = (char *)malloc(sizeof(char) * twoheapsz);
    if (!area) {
	fprintf(stderr, "ERROR Malloc failed!\n");
	exit(-1);
    }

    printf("Got heap area at addr=%p of size %lu\n", 
           area, (unsigned long)twoheapsz);
    ...
    pheap_down = umalloc_makeheap(area+twoheapsz-1, HEAP_SIZE, UMALLOC_HEAP_GROWS_DOWN);
    ...
    printf("\n%i: Running two-heap concurrent sizes test with %d iterations\n", id, iters); fflush(stdout);
    twoheap_sizes_test(pheap_up, pheap_down, iters);
