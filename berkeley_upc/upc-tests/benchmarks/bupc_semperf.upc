...
void memput_signal_unsafe(shared void *dst, void *src, size_t nbytes, bupc_sem_t *dummy, size_t dummy2) {
  static bupc_handle_t myhandle = BUPC_COMPLETE_HANDLE;
  if (myhandle != BUPC_COMPLETE_HANDLE) bupc_waitsync(myhandle);
  myhandle = bupc_memput_async(dst, src, nbytes);
  intflags[partnerid] = 1;    
}
...
int main(int argc, char **argv) {
  ...
  if (dosync && dopingpong && dosemt) { 
    ...
    bupc_tick_t ticks = bupc_ticks_now();
    ...
    ticks = bupc_ticks_now() - ticks;

    upc_barrier;

    if (!(MYTHREAD&1)) {
      double timeus = bupc_ticks_to_us(ticks);
...
if (dodata && dopingpong) {
 ...
 if (dointflags) {
  ...
  PINGPONG_PUT("upc_memput_async & strict int flag (UNSAFE)", memput_signal_unsafe, memput_wait_unsafe());
 }
}
  /* ------------------------------------------------------------------------------------ */
