...
int main(int argc, char **argv) {
  ...
  space = (char shared *)upc_all_alloc(1048576,10);
...
int doit(int iters) {
  ...
  #define TIMEIT(desc, code)                             \
    ...
    ((int32_t)(end-start))/(double)iters,        \
    ((int32_t)(end-start))/(double)1000000);

  if (MYTHREAD == 0) {
    TIMEIT("local        ptr-sum", mylptr = mylptr + i);
    TIMEIT("phased       ptr-sum", mysptr = mysptr + i);
    TIMEIT("phaseless(1) ptr-sum", myp1sptr = myp1sptr + i);
    TIMEIT("phaseless(I) ptr-sum", mypIsptr = mypIsptr + i);
