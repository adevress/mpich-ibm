...
int main(int argc, char **argv) {
  ...
  for (int iter = 0; iter < iters; iter++) {
    ...
    assert(upc_trysync(UPC_COMPLETE_HANDLE));
    upc_waitsync(UPC_COMPLETE_HANDLE);
    ...
    #define POPULATE() do {                                                      \
      for (int chunkidx=0; chunkidx < numchunks; chunkidx++) {                   \
        handles[chunkidx] =                                                      \
          upc_memput_async(peerarr1+ELEMOFFSET, mylarr2+ELEMOFFSET, CHUNKBYTES); \
      }                                                                          \
    } while (0)

    POPULATE();
    for (int i=0; i < numchunks; i++) upc_waitsync(handles[i]);

    POPULATE();
    for (int i=0; i < numchunks; i++) {
      while (!upc_trysync(handles[i])) upc_fence; 
    }
    ...
    DOTEST(handles[chunkidx] = upc_memput_async,
           handles[chunkidx] = upc_memget_async,
           handles[chunkidx] = upc_memcpy_async,
           handles[chunkidx] = upc_memset_async,
           {},upc_waitsync_all(handles, numchunks),"async");

    DOTEST(upc_memput_asynci,
           upc_memget_asynci,
           upc_memcpy_asynci,
           upc_memset_asynci,
           {},upc_waitsynci(),"asynci");
    ...
    DOTEST(upc_memput_asynci,
           ...
           upc_waitsync(upc_end_accessregion()),
           "accessregion");
