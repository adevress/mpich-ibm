 ...
 */
  ...
  #define UPCRI_ADDR_FITS_IN_SPTR   (UPCRI_ADDR_BITS >= (SIZEOF_VOID_P * 8))
    ...
    #define UPCRI_ADDR_BITS      (8*SIZEOF_VOID_P)
    #define UPCR_NULL_SHARED	      { 0, 0, 0 }
...
#define upcr_isvalid_pshared(sptr) _upcri_isvalid_pshared(sptr)
  ...
  upcr_shared_ptr_t  _upcri_checkvalid_shared(upcr_shared_ptr_t sptr,
                                             const char *filename, int linenum);
  upcr_pshared_ptr_t _upcri_checkvalid_pshared(upcr_pshared_ptr_t psptr,
                                             const char *filename, int linenum);
  #define upcri_checkvalid_shared(sptr)  _upcri_checkvalid_shared(sptr,__FILE__,__LINE__)
  #define upcri_checkvalid_pshared(sptr) _upcri_checkvalid_pshared(sptr,__FILE__,__LINE__)
 ...
 *    processes not using SysV shared memory.
 ...
 *    regions mmapped into each process (i.e. pthreads or SysV shmem used). 
 */
...
{
    /* TODO:  This function is a mess.  Clean up and optimize */
    /* DOB: note this code performs complicated arithmetic on a mix of 
       signed and unsigned types that have architecure-specific widths,
       and therefore relies on C99's integer promotion rules in some 
       very subtle ways -- when we clean it up, we should make these 
       implicit type conversions explicit for added clarity and ease 
    ...
    */
    ...
    long const bt = numthreads * blockelems;

    upcri_checkvalid_shared(sptr);
    upcri_assert(elemsz > 0);

    if (inc >= 0) {
	thread_incr = (inc + sptr_phase) / blockelems;
	out_phase = (inc + sptr_phase) % blockelems;
	out_thread = (sptr_thread + thread_incr) % numthreads;
	block_incr = (blockelems*sptr_thread + sptr_phase + inc) / bt;
	elemaddr_incr = ((ssize_t)out_phase - sptr_phase) + (block_incr*blockelems);
	out_addr = sptr_addr + (elemaddr_incr*(ssize_t)elemsz);
    } else {
	/* WARNING: minor changes here may break pgcc (see bug 1402) */
	/* TODO:  ldiv() may be slow: replace with faster logic
	 *  -- watch out for div of negative by unsigned: cast to signed first. */ 
	ldiv_t ld;
	long const phase_plus = sptr_phase + inc;

	ld = ldiv(phase_plus + (1 - (ssize_t)blockelems), blockelems);
	thread_incr = ld.quot;
	out_phase = ld.rem + (blockelems - 1);

	ld = ldiv(blockelems*sptr_thread + phase_plus + (1-bt), bt);
	block_incr = ld.quot; 

	ld = ldiv(sptr_thread + thread_incr + (1- (ssize_t)numthreads), numthreads);
	out_thread = ld.rem + (numthreads - 1);
...
{
    #if __GNUC__ == 3 && __GNUC_MINOR__ == 2 && __GNUC_PATCHLEVEL__ == 3 && defined(__x86_64__)
