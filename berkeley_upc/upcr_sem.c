 ...
 *
 * $Id: upcr_sem.c,v 1.26 2007/09/08 19:51:26 bonachea Exp $
...
#define _UPCRI_SEM_USERUP_ATOMIC(ps, flags, n) do {                     \
    upcri_sem_ctr_t oldval, newval;                                     \
    upcri_sem_ctr_t _inc = (n);                                         \
    UPCRI_CHECK_OVERFLOW(ps, flags, _inc);                              \
    if (UPCRI_SEM_IS_BOOLEAN(flags)) /* boolean up => just set */       \
      gasnett_atomic_set(&((ps)->userval.atomic_ctr),1,0);              \
    else if (_inc == 1) /* simple increment */                          \
      gasnett_atomic_increment(&((ps)->userval.atomic_ctr),0);          \
    else { /* general increment */                                      \
      do {                                                              \
        oldval = gasnett_atomic_read(&((ps)->userval.atomic_ctr),0);    \
        newval = oldval + _inc;                                         \
      } while (!upcri_cas(&((ps)->userval.atomic_ctr), oldval,newval,0)); \
    }                                                                   \
  } while (0)
#if GASNETT_HAVE_ATOMIC_CAS
...
static size_t upcri_sem_maxmed_threshold = 0;
  ...
  #elif defined(GASNET_CONDUIT_ELAN) && defined(GASNETC_ELAN3)
    static size_t upcri_sem_tinypacket_maxpayload = 255; /* empirically determined cross-over on flyer */
  #elif defined(GASNET_CONDUIT_ELAN)
    static size_t upcri_sem_tinypacket_maxpayload = 255; /* empirically determined cross-over on hive */
  #else
    static size_t upcri_sem_tinypacket_maxpayload = 32;
  #endif
  /* depth - depth of the tinypacket queue (0 == disable) */
  #if defined(GASNET_CONDUIT_GM) || \
      defined(GASNET_CONDUIT_MPI) || \
...
void _upcri_sem_init() {
  gasnet_hsl_lock(&upcri_init_hsl);
  if (!_upcri_sem_isinit) {

    /* choose an intelligent default */
    #if defined(GASNET_CONDUIT_VAPI) || defined(GASNET_CONDUIT_IBV)
      /* vapi's pipelined longs are just as efficient as mediums, so save the copy costs */
      upcri_sem_maxmed_threshold = 0;
    #elif defined(GASNET_CONDUIT_ELAN) && defined(GASNETC_ELAN3)
      upcri_sem_maxmed_threshold = MIN(gasnet_AMMaxMedium(), 16); /* empirically determined cross-over on flyer */
    #elif defined(GASNET_CONDUIT_ELAN)
      upcri_sem_maxmed_threshold = MIN(gasnet_AMMaxMedium(), 1024); /* empirically determined cross-over on n2001 */
    #elif defined(GASNET_CONDUIT_GM)
      upcri_sem_maxmed_threshold = MIN(gasnet_AMMaxMedium(), 2048); /* empirically determined cross-over on n2001 */
    #else
...
/* ------------------------------------------------------------------------------------ */
/* tinypacket support */
#define UPCRI_SEM_CAN_TINYPUT(flags) \
        (UPCRI_ALLOW_SINGLE_PUT && upcri_sem_tinypacket_depth)
...
void _upcri_sem_tinyaccept(upcri_sem_t *ps, int flags UPCRI_PT_ARG) {
  while (ps->queued_pst) { /* accept new connections */
    ...
    gasnet_hsl_unlock(&(ps->hsl));
    if (queued_pst) {
      upcr_shared_ptr_t allocptr = upcr_local_alloc(1, sizeof(upcri_semtarget_t) + 
...
int upcri_sem_tinyput(upcr_pshared_ptr_t sem, size_t seminc, gasnet_node_t node,
  ...
  if (idx >= myinfo->inittable_cnt || myinfo->inittable[idx] != ps) { /* first touch on this thread - init my local copy */
    ...
    { /* request a tinysem connection */
      ...
      upcr_shared_ptr_t allocptr = upcr_local_alloc(1, sizeof(upcri_semproducer_t) + 
             upcri_sem_tinypacket_depth*upcri_sem_tinypacket_bufsz + 
             2*GASNETT_CACHE_LINE_BYTES);
...
upcr_pshared_ptr_t _bupc_sem_alloc(int _flags UPCRI_PT_ARG) {
  ...
  #endif
  {
    allocptr = upcr_local_alloc(1, sizeof(upcri_sem_t)+UPCRI_SEM_ALIGN);
