...
shared void *upc_local_alloc_check(size_t a, size_t b, int line) {
  shared void *retval = upc_local_alloc(a,b);
  if ((void *)retval == NULL) {
    fprintf(stderr,"ERROR: failed to upc_local_alloc(%i,%i) at %s:%i\n", 
            (int)a, (int)b, __FILE__, line);
    fflush(stderr);
    upc_global_exit(-1);
  }
...
#define upc_local_alloc(a,b) upc_local_alloc_check((a),(b),__LINE__)
#endif
...
upc3d *create_upc3d(int nx,int ny,int nz,int level)
{
  ...
  result->totalSize = 1;
  result->sdims = (shared [] int *) upc_local_alloc(3,sizeof(int));
  for(i=0;i < 3;i++) {
  ...
  result->localData = (shared [] double *) 
                      upc_local_alloc(result->totalSize,sizeof(double));
#endif
  result->xzbuffer = (shared [] double *) 
                      upc_local_alloc(2*result->sdims[0]*result->sdims[2]+1,
                      sizeof(double));
  result->yzbuffer = (shared [] double *)
                      upc_local_alloc(2*result->sdims[1]*result->sdims[2]+1,
                      sizeof(double));
  result->xybuffer = (shared [] double *)
                      upc_local_alloc(2*result->sdims[0]*result->sdims[1]+1,
                      sizeof(double));
#ifdef ONLYBUFFER
...
#endif
  result->faceSync = (shared [] int *) upc_local_alloc(6,sizeof(int));
  for(i=0;i < 6;i++) {
    result->faceSync[i] = 0;
  }
  upc_barrier;
  ...
  if(MYTHREAD == 0) { 
    globaldir = upc_local_alloc(nn, sizeof(DIRTYPE));
    globalyzdir = upc_local_alloc(nn, sizeof(DIRTYPE));
    globalxzdir = upc_local_alloc(nn, sizeof(DIRTYPE));
    globalxydir = upc_local_alloc(nn, sizeof(DIRTYPE));
    globalfacedir = upc_local_alloc(nn,sizeof(DIRTYPE));
    globaldimsdir = upc_local_alloc(nn,sizeof(DIRTYPE));
  }
  upc_barrier;
