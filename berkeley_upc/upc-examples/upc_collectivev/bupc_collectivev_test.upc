/*   $Source: /var/local/cvs/upcr/upc-examples/upc_collectivev/bupc_collectivev_test.upc,v $ */
/*      $Date: 2007/03/06 19:44:49 $ */
/*  $Revision: 1.6 $ */
...
#define PRINT_LINE()  do { upc_barrier; if (MYTHREAD == 0) printf("-\n"); BARRIER(); } while (0)
...
int main(int argc, char **argv) {
   ...
   for (int i=0; i < THREADS; i++) {
     if (MYTHREAD != i) {
       ...
       int *result = calloc(THREADS,sizeof(int));
   ...
   for (int i=0; i < THREADS; i++) {
     ...
     if (MYTHREAD != i) {
       ...
       int *src = malloc(THREADS*sizeof(int));
       for (int j=0; j < THREADS; j++) {
         src[j] = FOO_VAL(j*10);
       }
   ...
   { /* test we can exchange pointers (in this case from all_alloc so we can 
     ...
     shared [1] char **result = calloc(THREADS,sizeof(shared [1] char *));
