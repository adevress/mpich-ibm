...
# $Id: upcc.cgi,v 1.15 2006/10/14 04:13:10 bonachea Exp $
################################################################################
...
sub runCmd {
    my ($cmd, $contextMsg) = @_;
    my $out = "upcc.stdout.tmp";
    my $err = "upcc.stderr.tmp";
    my $oldslash = $/;
    undef $/; # slurp!
    $runCmd_stdout = "";
    $runCmd_stderr = "";
    ...
    $runCmd_stdout = <GETOUT>;
    close GETOUT;
    open(GETERR, "$err") or die "Can't open temp file $err\n";
    my $runCmd_stderr = <GETERR>;  # slurped
    close GETERR;

    if ($exitval) {
        die "upcc.cgi: $contextMsg: $runCmd_stderr";
    } 
    $/ = $oldslash;
}
