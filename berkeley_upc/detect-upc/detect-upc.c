...
static int is_upc_file(const char * fname) {
    ...
    for (p = upc_files; p != NULL; p = p->next) {
   	if (!strcmp(fname, p->filename)) return 1;
    }
    return 0;
}	
...
{
    fseek(ifile, 0, SEEK_SET);	/* rewind input file */
    in_c = 0;
    do_output = 1;
    yylex();
}
...
void insert_pragma(char *yytxt)
{
    ...
    if (is_upc_file(filename)) {
	fprintf(ofile, "#pragma upc upc_code\n");
	/* Print #line directive again, else debugger/__LINE__ will be off
	 * by 1 */
    	fprintf(ofile, "%s", remove_ephemera_from_line(yytxt));
	if (verbose) {
	    printf("added UPC pragma to %s\n", filename);
	    fflush(stdout);
	}
	in_c = 0;
    } else {
        ...
        if (c_pragma && !in_c) {
    	    fprintf(ofile, "#pragma upc c_code\n");
	    /* Print #line directive again, else debugger/__LINE__ will be off
	     * by 1 */
	    fprintf(ofile, "%s", remove_ephemera_from_line(yytxt));
	    if (verbose) {
...
{
    fprintf(stdout, "Usage: %s -i dependency_file -o outfile [-b] [-v] preprocessed_input\n", 
	    argv0);
    exit(exitcode);
}
...
{   
    ...
    while (fscanf(depfile, "%s %d", temp, &num_of_includers) > 0) {
...
int main(int argc, char **argv)
{
    ...
    while ( (flag = getopt(argc, argv, "bhvi:o:")) != -1) {
	switch (flag) {
	case 'h':
