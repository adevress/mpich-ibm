 ...
 * $Id: upcr_coll_templates.c,v 1.23 2006/03/19 02:07:52 bonachea Exp $
 */
 ...
 * UPCRI_COLL_RED_OPS:		legal operators, one of UPCRI_COLL_RED_{INT,NONINT}
 *
 */

#if !defined(UPCRI_COLL_RED_TYPE) || !defined(UPCRI_COLL_RED_SUFF) || !defined(UPCRI_COLL_RED_OPS)
  #error "This file is only for inclusion by upcr_collective.c"
#endif

/* The following supporting functions and/or macros are defined only on first inclusion */
#ifndef _UPCR_COLL_TEMPLATES_C
  #define _UPCR_COLL_TEMPLATES_C 1

  static const char *_upcri_op2str(upc_op_t op) {
      switch (op) {
  ...
  #define UPCRI_TRACE_RED(name, type, op, nelems) \
        UPCRI_TRACE_PRINTF(("COLLECTIVE UPC_ALL_" _STRINGIFY(name) ": type = %2s count = %6llu op = %s", _STRINGIFY(type), (long long unsigned)(nelems), _upcri_op2str(op)))
  ...
  #else
    /* Anonymous barriers for speed */
    #define UPCRI_COLL_BARRIER_DECL() \
	int _dummy_to_eat_semicolon = 0
    #define UPCRI_COLL_NOTIFY_IF(cond) \
	if_pf(cond) { _upcr_notify(0, UPCR_BARRIERFLAG_ANONYMOUS UPCRI_PT_PASS); }
    #define UPCRI_COLL_WAIT_IF(cond) \
	if_pf(cond) { _upcr_wait(0, UPCR_BARRIERFLAG_ANONYMOUS UPCRI_PT_PASS); }
  #endif
...
UPCRI_COLL_REDUCE_NC_PROTO(UPCRI_COLL_RED_TYPE,UPCRI_COLL_RED_SUFF)
{
...
#else
    partial_sptr = upcr_local_alloc(partials, sizeof(UPCRI_COLL_RED_TYPE));
	 ...
	 */
	if (dst_th == mythread) {
	    gath_sptr = upcr_local_alloc(upcri_threads * partials, sizeof(UPCRI_COLL_RED_TYPE));
...
UPCRI_COLL_REDUCE_C_PROTO(UPCRI_COLL_RED_TYPE,UPCRI_COLL_RED_SUFF)
{
    ...
    my_partial = &my_temp;
#else
    partial_sptr = upcr_local_alloc(1, sizeof(UPCRI_COLL_RED_TYPE));
    my_partial = upcr_shared_to_local(partial_sptr);
#endif
	...
	if (dst_th == mythread) {
	    gath_sptr = upcr_local_alloc(upcri_threads, sizeof(UPCRI_COLL_RED_TYPE));
	    gathered = upcr_shared_to_local(gath_sptr);
	}
	gasnet_coll_gather(GASNET_TEAM_ALL, dst_th, gathered, my_partial,
			   sizeof(UPCRI_COLL_RED_TYPE),
...
UPCRI_COLL_PREFIX_REDUCE_PROTO(UPCRI_COLL_RED_TYPE,UPCRI_COLL_RED_SUFF)
{
	...
	if (my_count != 0) {
		...
		if (upcri_thread_is_local(src_th)) {
		    UPCRI_UNALIGNED_MEMCPY(my_dst, my_src, nbytes);
		} else {
