 ...
 *
 * $Id: upcr_totalview.c,v 1.8 2007/01/25 07:00:04 bonachea Exp $
 */
...
_upcri_startup_totalview(char **argv UPCRI_PT_ARG)
{
...
#endif
  {
    ...
    #else
      myph_sptr = upcr_local_alloc(1, sizeof(struct pid_and_host));
    ...
    if (gasnet_mynode() == 0) {
      ...
      allph_sptr = upcr_local_alloc(gasnet_nodes(), sizeof(struct pid_and_host));
