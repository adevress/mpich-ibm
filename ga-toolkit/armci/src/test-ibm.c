...
#define BASE 100.
#define MAXPROC 8192 /* max allowed by short int in armci structures */
#define TIMES 100
...
void test_dim(int ndim)
{
    ...
    //printf("DEBUG before create arrays\n");
    ...
    {
        int idx1, idx2, idx3;
        //printf("DEBUG before get_range %d\n", i);
        get_range(ndim, dimsA, loA, hiA);
        //printf("DEBUG before new_range %d\n", i);
...
void test_swap()
{
    ...
    for(i = 0; i < LOOP; i++)
    {
        ...
        do
        {
            ARMCI_CRITICAL_SECTION_ENTER();
            rc = ARMCI_Rmw(ARMCI_SWAP, &val, arr[0], whatever, 0);
            assert(rc == 0);
            if(val == LOCKED) ARMCI_CRITICAL_SECTION_EXIT();
        } while (val == LOCKED); 
        val++;
        rc = ARMCI_Rmw(ARMCI_SWAP, &val, arr[0], whatever, 0);
        ARMCI_CRITICAL_SECTION_EXIT();
        assert(rc == 0);
    }
...
void test_memlock()
{
    ...
    { 
        // for(i=0;i<TIMES*nproc;i++)
        // { 
                                                /* assuming ELEMS is 200 */
            first = rand() % (4 * (ELEMS / 5)); /* first is random number 0 -> 159 */
            last = first + (ELEMS / 5) - 1;     /* last is first + 39 */
            pstart = b[proc] + first;           /* pstart is b[0] plus offset first */
            pend = b[proc] + last + 1;          /* psend is b[0] plus first offset plus 40 */
            elems = last - first + 1;           /* elems is 40 */
            bytes = sizeof(double) * elems;     /* bytes the size of 40 doubles */

            armci_lockmem(pstart,pend,proc);
            assert(!ARMCI_Put(a, pstart, bytes, proc));

            /* fence is required to ensure remote completion of the blocking put operation */
            ARMCI_Fence(proc);
...
int main(int argc, char* argv[])
{
    ...
    #define TEST_DIM 0
    #define TEST_NBDIM 1
    #define TEST_VEC_SMALL 2
    #define TEST_ACC 3
    #define TEST_VECTOR 4
    #define TEST_VECTOR_ACC 5
    #define TEST_FETCH_ADD 6
    #define TEST_SWAP 7
    #define TEST_RPUT 8
    #define TEST_AGGREGATE 9
    #define TEST_IMPLICIT 10
    #define TEST_MEMLOCK 11
    #define TEST_ACC_TYPE 12
    #define TEST_COLLECTIVE 13
...
*/
    ...
    gettimeofday(&start_time[TEST_DIM],NULL);
    if(test_flags[TEST_DIM] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_DIM],NULL);

    gettimeofday(&start_time[TEST_NBDIM],NULL);
    if(test_flags[TEST_NBDIM] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_NBDIM],NULL);

    gettimeofday(&start_time[TEST_VEC_SMALL],NULL);
    if(test_flags[TEST_VEC_SMALL] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_VEC_SMALL],NULL);

    gettimeofday(&start_time[TEST_ACC],NULL);
    if(test_flags[TEST_ACC] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_ACC],NULL);

    gettimeofday(&start_time[TEST_VECTOR],NULL);
    if(test_flags[TEST_VECTOR] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_VECTOR],NULL);

    gettimeofday(&start_time[TEST_VECTOR_ACC],NULL);
    if(test_flags[TEST_VECTOR_ACC] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_VECTOR_ACC],NULL);

    gettimeofday(&start_time[TEST_FETCH_ADD],NULL);
    if(test_flags[TEST_FETCH_ADD] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_FETCH_ADD],NULL);

    gettimeofday(&start_time[TEST_SWAP],NULL);
    if(test_flags[TEST_SWAP] == 1 || test_all_flag == 1)
    {
        if(me==0)
        {
            printf("\nTesting atomic swap\n");
    ...
    gettimeofday(&stop_time[TEST_SWAP],NULL);

    gettimeofday(&start_time[TEST_RPUT],NULL);
    if(test_flags[TEST_RPUT] == 1 || test_all_flag == 1)
    {
        if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_RPUT],NULL);

    gettimeofday(&start_time[TEST_AGGREGATE],NULL);
    //if(test_flags[TEST_AGGREGATE] == 1 || test_all_flag == 1)
    if(0)
    {
	    if(me == 0)
        {
	...
	*/
    ...
    gettimeofday(&stop_time[TEST_AGGREGATE],NULL);

    gettimeofday(&start_time[TEST_IMPLICIT],NULL);
    if(test_flags[TEST_IMPLICIT] == 1 || test_all_flag == 1)
    {
	    if(me == 0)
        {
    ...
    gettimeofday(&stop_time[TEST_IMPLICIT],NULL);

    gettimeofday(&start_time[TEST_MEMLOCK],NULL);
    if(test_flags[TEST_MEMLOCK] == 1 || test_all_flag == 1)
    {
	    if(me == 0)
        {
	        printf("\nTesting memory lock\n");
    ...
    gettimeofday(&stop_time[TEST_MEMLOCK],NULL);

    gettimeofday(&start_time[TEST_ACC_TYPE],NULL);
    if(test_flags[TEST_ACC_TYPE] == 1 || test_all_flag == 1)
    {
	    if(me == 0)
        {
    ...
    if(test_flags[TEST_COLLECTIVE] == 1 || test_all_flag == 1)
    {
	    if(me == 0)
        {
