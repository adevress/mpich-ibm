...
extern long armci_shm_reg_size(int , long );
...
#define IN_REGION(_ptr__,_reg__) ((_ptr__)>=(_reg__.serv_ptr) \
...
int rc,i,j;
    ARMCI_PR_SDBG("enter",0);
    serv_bufs=(serv_buf_t *)calloc(1,sizeof(serv_buf_t));
    assert(serv_bufs);
    serv_bufs->buf=(char *)calloc(1,(NUM_SERV_BUFS*armci_nproc*VBUF_DLEN));
...
int temp_proc;
    ...
    while(ar->val){
      ...
      abort();
/*cedo        armci_die("event failure problem",0);*/
      }
      if(DEBUG_COMM){
...
request_header_t *msginfo = (request_header_t *)buf;
    ...
    * %p",armci_me,tag,c_info,cdesc);fflush(stdout);*/
    ...
    if(msginfo->operation==PUT || msginfo->operation == UNLOCK || ACC(msginfo->operation)){
...
request_header_t *msginfo = (request_header_t *)mesg;
    ...
    *(void **)phdr = msginfo;
    if(DEBUG_COMM){
        printf("%d(server): got %d req (dscrlen=%d datalen=%d %d) from %d\n",
               armci_me, msginfo->operation, msginfo->dscrlen,
               msginfo->datalen, msginfo->bytes,msginfo->from); fflush(stdout);
    }
...
int temp_proc;
    ...
    while(armci_server_terminating==0){
      ...
      if (ev->ni_fail_type != PTL_NI_OK) {
        printf("(%d):NI sent %d in event.\n",
                        portals->Srank,  ev->ni_fail_type); 
      abort();
/*cedo        armci_die("event failure problem",0);*/
      }
      if(DEBUG_SERV){
        printf("\n(%d):armci_call_data_server:done waiting type=%d\n",armci_me,
                        ev->type);
        fflush(stdout);
      }
      ...
      if (ev->type != PTL_EVENT_PUT_END){
        ...
        armci_die("unexpected event",ev->type);
      }
      if(DEBUG_SERV){printf("\n(%d):ev->offset=%d from %d%d",armci_me,ev->offset,ev->initiator.pid,ev->initiator.nid);fflush(stdout);}
