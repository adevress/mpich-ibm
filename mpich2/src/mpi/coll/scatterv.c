	...
	MPID_Comm *comm_ptr )
{
    ...
    if ((comm_ptr->comm_kind == MPID_INTRACOMM) && (rank == root)) {
        ...
        /* I need to add a check to ensure extent will fit in a pointer... We
         * need extent * (max count) but i can't get that without looping over
         * the input data. i don't think that is worth doing. this is at least
