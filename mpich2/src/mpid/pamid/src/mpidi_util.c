...
void MPIDI_print_statistics() {
  ...
  extern pami_extension_t progress_extension;
  typedef enum
   {
     PAMI_ASYNC_ALL            =    0,
  ...
  typedef pami_result_t (* async_progress_disable_fn) (pami_context_t context,
                                                 pami_async_t  event_type);
  async_progress_disable_fn  pe_progress_disable=NULL;
  typedef void (* pamix_async_fn) (pami_context_t context, void *cookie);

  typedef pami_result_t (* async_progress_register_fn) (pami_context_t   context,
                                                        pamix_async_fn   progress_fn,
                                                        pamix_async_fn   suspend_fn,
                                                        pamix_async_fn   resume_fn,
                                                        void             * cookie);
  async_progress_register_fn  pe_progress_register=NULL;

void MPIDI_open_async_extension() {
    int rc;
     /* open EXT_async_progress extension       */
     memset(&progress_extension,0, sizeof(pami_extension_t));
     rc = PAMI_Extension_open (MPIDI_Client, "EXT_async_progress", &progress_extension);
     TRACE_ERR("PAMI_Extension_open: rc %d EXT_async_progress\n", rc);
     if (rc != PAMI_SUCCESS) {
         TRACE_ERR("ERROR open PAMI_Extension_open async_extension failed rc %d", rc);
         MPID_assert_always(rc == PAMI_SUCCESS);
     }
}

void MPIDI_disable_ASYNC() {
  int rc;
  pe_progress_disable = (async_progress_disable_fn )PAMI_Extension_symbol(progress_extension, "disable");
  if (!pe_progress_disable) {
      TRACE_ERR("ERROR pe_progress_disable==NULL ");
  }
  rc=pe_progress_disable(MPIDI_Context[0],PAMI_ASYNC_ALL);
  if (rc != PAMI_SUCCESS) {
      TRACE_ERR("ERROR close PAMI_Extension ASYNC_ALL failed rc %d", rc);
  }
}

void MPIDI_close_async_extension() {
     int rc;
     rc = PAMI_Extension_close (progress_extension);
     if (rc != PAMI_SUCCESS) {
         TRACE_ERR("ERROR close PAMI_Extension progress_extension failed rc %d", rc);
     }
}
