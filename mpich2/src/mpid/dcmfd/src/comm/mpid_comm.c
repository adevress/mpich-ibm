...
#warning someone needs to update this description
  ...
  if(envopts != NULL)
  {
    if(strncasecmp(envopts, "N", 1) == 0)
    {
      DCMF_INFO_UNSET(properties, DCMF_USE_PREMALLOC_REDUCE);
      DCMF_INFO_UNSET(properties, DCMF_USE_ALLREDUCE_REDUCE);
    }
    else if(strncasecmp(envopts, "Y", 1) == 0) /* defaults to Y */
    {
      DCMF_INFO_SET(properties, DCMF_USE_PREMALLOC_REDUCE);
      DCMF_INFO_SET(properties, DCMF_USE_ALLREDUCE_REDUCE);
    }
    else
